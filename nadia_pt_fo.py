...
class ModusTollensDef():
    ...
    def evaluation(self, parser, deduction_result):
        ...
        # Caso 2: 
        if isinstance(formula2, BinaryFormula) and formula2.is_implication():
            p = formula2.left
            q = formula2.right

            if formula1 == NegationFormula(q) and self.formula == NegationFormula(p):
                valid = True
...
class DisjunctionRuleDef():
    ...
    def evaluation(self, parser, deduction_result):
        ...
        # formula2 = ~P
        # conclusão = Q
        if isinstance(formula1, BinaryFormula) and formula1.is_disjunction():
            p = formula1.left
            q = formula1.right

            if formula2 == NegationFormula(p) and self.formula == q:
                valid = True

            if formula2 == NegationFormula(q) and self.formula == p:
                valid = True
        ...
        # formula1 = ~P
        # conclusão = Q
        if isinstance(formula2, BinaryFormula) and formula2.is_disjunction():
            p = formula2.left
            q = formula2.right

            if formula1 == NegationFormula(p) and self.formula == q:
                valid = True

            if formula1 == NegationFormula(q) and self.formula == p:
                valid = True
...
class DoubleNegationIntroductionDef():
    ...
    def evaluation(self, parser, deduction_result):
        ...
        expected = NegationFormula(NegationFormula(formula1))
...
class DoubleNegationEliminationDef():
    def __init__(self, line, formula, reference1):
        self.line = line
        self.formula = formula
        self.reference1 = reference1
    ...
    def evaluation(self, parser, deduction_result):
        ...
        if isinstance(formula1, NegationFormula):
            inner = formula1.formula

            if isinstance(inner, NegationFormula):
                if self.formula == inner.formula:
                    valid = True
        ...
        def program(p):
            ...
            for i in rule_info:
                ...
                if(isinstance(rule, PremisseDef) ):
                    ...
                    rule.evaluation(self, deduction_result)
                elif isinstance(rule, DoubleNegationEliminationDef):
                    rule.evaluation(self, deduction_result)
...
##                print(deduction_result.fitch)
        ...
        @self.pg.production('step : NUM DOT formula MODUS_TOLLENS NUM COMMA NUM')
        def Modus_tollens(p):
            formula_result = p[2]
            formula = formula_result[1]

            modusTollens = ModusTollensDef(p[0].value, formula, p[4], p[6])
            self.symbol_table.insert(modusTollens, p[0])
        ...
        @self.pg.production('step : NUM DOT formula DOUBLE_NEG_INTROD NUM')
        def Double_negation_introduction(p):
            formula_result = p[2]
            formula = formula_result[1]

            doubleNegationIntroduction = DoubleNegationIntroductionDef(
                p[0].value,
                formula,
                p[4]
            )

            self.symbol_table.insert(doubleNegationIntroduction, p[0])
        ...
        @self.pg.production('step : NUM DOT formula DOUBLE_NEG_ELIM NUM')
        def Double_negation_elimination(p):
            formula_result = p[2]
            formula = formula_result[1]

            doubleNegationElimination = DoubleNegationEliminationDef(
                p[0].value,
                formula,
                p[4]
            )

            self.symbol_table.insert(doubleNegationElimination, p[0])
